...
# Read pose from topic and broadcast it to /tf for a fixed amount of time
...
class Boostrapper:
    ...
    def handle_dvs_pose(self, msg):
        ...
        if not self.got_first_pose:
            # print('Broadcasting pose to tf frame: %s%s'.format(self.tf_world_name, self.tf_frame_name))

            # Keep first pose T_w_dvs to use as the reference coordinate system
            t = msg.pose.position
            q = msg.pose.orientation
            self.T_w_init = transformations.matrix_from_quaternion(np.array([q.x, q.y, q.z, q.w]))
            self.T_w_init[:3, 3] = np.array([t.x, t.y, t.z])

            self.got_first_pose = True

        # Transform the DVS pose into the reference coordinate system defined by the first pose
        t = msg.pose.position
...
if __name__ == '__main__':
    ...
    bootstrapper = Boostrapper(tf_frame_name, tf_world_name, relative_to_first_pose)
