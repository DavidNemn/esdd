...
// #define DEBUG_SHOW_FAKE_FRAMES       // 显示事件累积图像
// #define DEBUG_SHOW_FILTERED_DEPTHMAP // 显示滤波后的深度图
...
namespace depth_from_defocus
{
    ...
    DepthFromDefocusNode::DepthFromDefocusNode(const ros::NodeHandle &nh, const ros::NodeHandle &pnh, const image_geometry::PinholeCameraModel &cam)
        : nh_(nh), pnh_(pnh), it_(nh), state_(IDLE), dvs_cam_(cam), pc_(new PointCloud), pc_global_(new PointCloud)
    {
        radius_search_ = rpg_common_ros::param<float>(pnh_, "radius_search", 0.05);
        min_num_neighbors_ = rpg_common_ros::param<int>(pnh_, "min_num_neighbors", 3);
        median_filter_size_ = rpg_common_ros::param<int>(pnh_, "median_filter_size", 7);
        events_to_recreate_kf_ = rpg_common_ros::param<int>(pnh_, "events_to_recreate_kf", 2000000);

        regular_frame_id_ = rpg_common_ros::param(nh_, "dvs_frame_id", std::string("dvs"));
        bootstrap_frame_id_ = rpg_common_ros::param(nh_, "dvs_bootstrap_frame_id", regular_frame_id_);
        frame_id_ = bootstrap_frame_id_;
        world_frame_id_ = rpg_common_ros::param<std::string>(nh_, "world_frame_id", "world");
        ...
        event_sub_ = nh_.subscribe("events", 0, &DepthFromDefocusNode::processEventArray, this);
        remote_key_ = nh_.subscribe("remote_key", 0, &DepthFromDefocusNode::remoteKeyCallback, this);
        ...
        auto_trigger_ = rpg_common_ros::param<bool>(pnh_, "auto_trigger", true);
        if (auto_trigger_)
            state_ = MAPPING;
    }
    ...
    void DepthFromDefocusNode::postCameraLoaded()
    {
        ...
        else
        {
            ...
            f_virtual_cam_ = 0.5 * (float)virtual_width_ / std::tan(0.5 * fov_virtual_cam_rad);
        }
        LOG(INFO) << "Focal length of virtual camera: " << f_virtual_cam_ << " pixels";
        virtual_cam_ = PinholeCamera(virtual_width_, virtual_height_, f_virtual_cam_, f_virtual_cam_, 0.5 * (float)virtual_width_, 0.5 * (float)virtual_height_);

        K_ << dvs_cam_.fx(), 0.f, dvs_cam_.cx(), 0.f, dvs_cam_.fy(), dvs_cam_.cy(), 0.f, 0.f, 1.f;
    ...
    {
        if (state_ == IDLE) // if (state_ == IDLE && !auto_trigger)
            return;

        for (const geometry_msgs::TransformStamped &transform_stamped_msg : tf_msg->transforms)
        {
            ...
            {
                while (newest_tracked_event_ < event_queue_.size() && event_queue_[newest_tracked_event_ + 1].ts < transform_stamped_msg.header.stamp)
                ...
                if (auto_trigger_ && (newest_tracked_event_ - current_event_ > 2 * events_to_recreate_kf_)) // if (auto_trigger_ && (newest_tracked_event_ - current_event_ > 2 * events_to_recreate_kf_))
                {
                    update(); // 更新地图
                    auto_trigger_ = false;
                    LOG(INFO) << "初始化地图成功!";
                    frame_id_ = regular_frame_id_; // 累积够events_to_recreate_kf_个事件的位姿才会切换到regular_frame_id_
                }
            }
        }
    }
    ...
    void DepthFromDefocusNode::remoteKeyCallback(const std_msgs::String::ConstPtr &msg)
    {
        ...
        {
        case MAPPING:
            if (command_str == "update") // 收到来自trigger_map_expansion.py发送的消息
            {
#ifdef MAPPING_PERF
                TIMER_START(t1);
#endif
                ...
                LOG(INFO) << "[Map update] comp. time: " << duration << " milliseconds";
#endif
            }
            else if (command_str == "reset")
            {
                LOG(INFO) << "Resetting mapper. Will continue to respond to 'update' requests";
                resetMapper();
            }
            else if (command_str == "disable_map_expansion")
            {
                LOG(INFO) << "Switching mapper to IDLE mode. Will stop treating 'update' requests";
                resetMapper();
                state_ = IDLE;
            }
            break;
        ...
        case IDLE:
        default:
            if (command_str == "reset" || command_str == "enable_map_expansion" || command_str == "bootstrap")
            {
                state_ = MAPPING;
                if (command_str == "bootstrap")
                    auto_trigger_ = true;
                LOG(INFO) << "Swiching to MAPPING mode";
            }
            else if (command_str == "update" || command_str == "switch")
            {
                LOG(INFO) << "Ignoring command '" << command_str << "' because I am in IDLE mode";
            }
            break;
        }
    }
    ...
    {
        static const size_t skip_batches_normal = rpg_common_ros::param<int>(pnh_, "skip_batches", 0);          // 跳过的batch(投影事件组)数目
        static const size_t skip_batches_reset = rpg_common_ros::param<int>(pnh_, "skip_batches_for_reset", 0); // 重置DSI时跳过的batch(投影事件组)数目
        static const size_t max_event_rate = rpg_common_ros::param<int>(pnh_, "max_event_rate", 10000000);
        static const size_t frame_size = rpg_common_ros::param<int>(pnh_, "frame_size", 1024);
        static const size_t min_batch_size = rpg_common_ros::param<int>(pnh_, "min_batch_size", 20000); // 小于min_batch_size就不投影

        // Performance measurement
        static const size_t perf_interval = 1000000;
        static size_t events_processed = 0;
        static double last_ts = event_queue_[current_event_].ts.toSec(), time_evolved = 0;

        const size_t n_events = newest_tracked_event_ - current_event_;

        if (n_events < min_batch_size)
            return;

        size_t skip_batches = (reset) ? skip_batches_reset : skip_batches_normal;

        // Adapt skip_batches to max_event_rate
        const double duration = (event_queue_[newest_tracked_event_].ts - event_queue_[current_event_].ts).toSec();
        ...
        while (current_event_ + (skip_batches + 1) * frame_size < newest_tracked_event_)
        {
            ...
            const evo_utils::geometry::Transformation T_cur_ref = T_ref_cur.inverse();
    ...
    void DepthFromDefocusNode::projectEventsToVoxelGrid(const std::vector<Eigen::Vector4f> &events, const std::vector<Eigen::Vector3f> &centers)
    {
...
#pragma omp parallel for if (events.size() >= 20000)
    ...
    void DepthFromDefocusNode::resetVoxelGrid()
    {
        ...
        current_event_ = (newest_tracked_event_ > events_to_recreate_kf_) ? newest_tracked_event_ - events_to_recreate_kf_ : 0u;

        // Center the voxel grid in the middle pose of the sequence of events used to fill the DSI.
    ...
    {
        evo_utils::geometry::Depth min_depth = rpg_common_ros::param<float>(nh_, "min_depth", 1.0);
        evo_utils::geometry::Depth max_depth = rpg_common_ros::param<float>(nh_, "max_depth", 10.0);
        num_depth_cells_ = (size_t)rpg_common_ros::param<int>(nh_, "num_depth_cells", 64);

        adaptive_threshold_kernel_size_ = rpg_common_ros::param<int>(pnh_, "adaptive_threshold_kernel_size", 5);

        adaptive_threshold_c_ = rpg_common_ros::param<int>(pnh_, "adaptive_threshold_c", -10);
        ...
        ref_voxel_grid_.resize(num_depth_cells_ * virtual_width_ * virtual_height_, 0);
    }

    void DepthFromDefocusNode::processEventArray(const dvs_msgs::EventArray::ConstPtr &event_array)
    {
        if (state_ == IDLE)
        {
            return;
        ...
        {
            tf::StampedTransform T(tf::Transform::getIdentity(), event_queue_[0].ts, world_frame_id_, frame_id_);
            tf_->setTransform(T);
    ...
    void DepthFromDefocusNode::clearEventQueue()
    {
        ...
        if (last_event > event_history_size_)
        {
            ...
            event_queue_.erase(event_queue_.begin(), event_queue_.begin() + remove_events);
    ...
    void DepthFromDefocusNode::synthesizeAndPublishDepth()
    {
        ...
        accumulatePointcloud(depth, confidence_mask_, confidence,
                             T_w_ref.getRotation().getRotationMatrix(), T_w_ref.getPosition(),
    ...
    bool DepthFromDefocusNode::getPoseAt(const ros::Time &t, evo_utils::geometry::Transformation &T)
    {
    ...
    void DepthFromDefocusNode::synthesizePointCloudFromVoxelGridContrast(cv::Mat &depth_cell_indices, cv::Mat &confidence)
    {
        // Bell-shaped mask for a patch
        const int half_patchsize = rpg_common_ros::param<int>(pnh_, "half_patchsize", 3); // 获取深度切片时去除边缘像素
        ...
        {
            // Get depth plane
            Vote *pgrid = &ref_voxel_grid_[depth_plane * virtual_width_ * virtual_height_];
            ...
            cv::GaussianBlur(slice_dsi, slice_local_mean, cv::Size(patch_size, patch_size), 0, 0);
            cv::GaussianBlur(slice_dsi.mul(slice_dsi), slice_local_MS, cv::Size(patch_size, patch_size), 0, 0);
            slice_local_var = slice_local_MS - slice_local_mean.mul(slice_local_mean);

            // Retain maximum local variance (contrast) per pixel (i.e., per optical ray)
            for (int y = half_patchsize; y < virtual_height_ - half_patchsize; ++y)
            {
                for (int x = half_patchsize; x < virtual_width_ - half_patchsize; ++x)
                {
                    ...
                    {
                        confidence.at<float>(y, x) = contrast; // max of contrast along optical ray
    ...
    void DepthFromDefocusNode::synthesizePointCloudFromVoxelGridGradMag(cv::Mat &depth_cell_indices, cv::Mat &confidence)
    {
        // Bell-shaped mask for a patch
        const int half_patchsize = rpg_common_ros::param<int>(pnh_, "half_patchsize", 3);
        const int patch_size = 2 * half_patchsize + 1;
    ...
    void DepthFromDefocusNode::synthesizePointCloudFromVoxelGridLinf(cv::Mat &depth_cell_indices, cv::Mat &confidence)
    {
        ...
        for (int y = 0; y < virtual_height_; ++y)
        {
            for (int x = 0; x < virtual_width_; ++x)
            {
                ...
                for (size_t depth_plane = 0; depth_plane < num_depth_cells_; ++depth_plane)
                {
                    num_votes_vec.at(depth_plane) = voxelGridAt(ref_voxel_grid_, x, y, depth_plane);
                }

                // Look for inv_depth* = argmax(sharpness(inv_depth))
                auto max = std::max_element(std::begin(num_votes_vec), std::end(num_votes_vec));

                confidence.at<float>(y, x) = *max; // max number of votes along optical ray
                depth_cell_indices.at<uchar>(y, x) = std::distance(std::begin(num_votes_vec), max);
            }
        }
    }

    void DepthFromDefocusNode::synthesizePointCloudFromVoxelGrid(cv::Mat &depth, cv::Mat &confidence)
    {
        confidence = cv::Mat::zeros(virtual_height_, virtual_width_, CV_32F);
        cv::Mat depth_cell_indices = cv::Mat::zeros(virtual_height_, virtual_width_, CV_8U);

        // Choose method to obtain depth and confidence maps from the DSI
        static const int type_focus_measure = rpg_common_ros::param<int>(pnh_, "type_focus_measure", 0);
        ...
        if (type_focus_measure == 1)
        {
            ...
            synthesizePointCloudFromVoxelGridContrast(depth_cell_indices, confidence);
        }
        else if (type_focus_measure == 2)
        {
            ...
            synthesizePointCloudFromVoxelGridGradMag(depth_cell_indices, confidence);
        }
        else
        {
        ...
        evo_utils::geometry::huangMedianFilter(depth_cell_indices, depth_cell_indices_filtered, confidence_mask_, median_filter_size_);
        //  LOG(INFO) << "Done in " << median_filtering_timer.stop() << " s";

        depth = cv::Mat(virtual_height_, virtual_width_, CV_32F, cv::Scalar(max_depth_));
        for (int y = 0; y < virtual_height_; ++y)
        {
            for (int x = 0; x < virtual_width_; ++x)
            {
                depth.at<float>(y, x) = depths_vec_.cellIndexToDepth(depth_cell_indices_filtered.at<uchar>(y, x));
            }
        }
    ...
    void DepthFromDefocusNode::accumulatePointcloud(const cv::Mat &depth, const cv::Mat &mask, const cv::Mat &confidence,
                                                    ...
                                                    const ros::Time &timestamp)
    {
        ...
        for (int y = 0; y < virtual_height_; ++y)
        {
            for (int x = 0; x < virtual_width_; ++x)
            {
                ...
                {
                    BearingVector f_ref = virtual_cam_.projectPixelTo3dRay(Keypoint(x, y));
                    f_ref.normalize();
                    Eigen::Vector3d xyz_world = R_world_ref * (f_ref / f_ref[2] * depth.at<float>(y, x)) + t_world_ref;
    ...
    void DepthFromDefocusNode::publishDepthmap(const cv::Mat &depth, const cv::Mat &mask)
    {
    ...
    void DepthFromDefocusNode::publishVoxelGrid(const evo_utils::geometry::Transformation &T_w_ref) const
    {
        if (pub_voxel_grid_.getNumSubscribers() > 0)
        {
            ...
            array.layout.dim[0].stride = num_depth_cells_ * virtual_height_ * virtual_width_;
    ...
    {
        static const int accumulate_once_every = rpg_common_ros::param<int>(pnh_, "accumulate_local_map_once_every", 1);
        static const int skip_first = rpg_common_ros::param<int>(pnh_, "global_point_cloud_skip_first", 0);
        static int accumulation_cnt = 0;
        static int skipped = 0;

        if (pub_pc_global_.getNumSubscribers() > 0 && skipped >= skip_first && (accumulation_cnt % accumulate_once_every == 0))
        {
            *pc_global_ += *pc_;
            ...
            static const float voxel_filter_leaf_size = rpg_common_ros::param<float>(pnh_, "voxel_filter_leaf_size", .0f); // 实行体素滤波
            ...
            if (voxel_filter_leaf_size > 0.0001f)
            {
                ...
                sor.setLeafSize(voxel_filter_leaf_size, voxel_filter_leaf_size, voxel_filter_leaf_size);
            ...
            static const float radius_search = rpg_common_ros::param<float>(pnh_, "radius_search_global_map", .02);
            static const int min_num_neighbors = rpg_common_ros::param<int>(pnh_, "min_num_neighbors_global_map", 5);
