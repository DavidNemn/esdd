...
namespace evo_utils::camera
{
    ...
    image_geometry::PinholeCameraModel loadPinholeCamera(ros::NodeHandle &nh)
    {
        ...
        // 使用camera_name变量在calib_file中找到相机标定信息
        const std::string camera_name = rpg_common_ros::param<std::string>(nh, "camera_name", "");

        const std::string calib_file = rpg_common_ros::param<std::string>(nh, "calib_file", "");
