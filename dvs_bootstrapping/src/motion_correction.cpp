...
namespace motion_correction
{
                                ...
                                std::vector<cv::Point2f> rectified_points)
    {
        ...
        if (!pts_begin_initialized)
        {
            ...
            for (size_t y = 0; y < sensor_size.height; ++y)
            {
                ...
                {
                    const cv::Point2f &p = rectified_points[x + y * sensor_size.width];
                    pts_begin_2x1.at<cv::Vec2f>(y, x) = cv::Vec2f(p.x, p.y);
                    pts_begin_homogeneous_3x1.at<cv::Vec3f>(y, x) = cv::Vec3f(p.x, p.y, 1.f);
                }
            }
        ...
        for (size_t y = 0; y < sensor_size.height; ++y)
        {
            for (size_t x = 0; x < sensor_size.width; ++x)
            {
                ...
                if (is_homography)
                {
                    flow = (pts_end.at<cv::Vec2f>(y, x) - pts_begin->at<cv::Vec2f>(y, x)) / dt;
                }
                else
                {
                    flow[0] = (pts_end.at<cv::Vec2f>(y, x)[0] - pts_begin->at<cv::Vec3f>(y, x)[0]) / dt;
                    flow[1] = (pts_end.at<cv::Vec2f>(y, x)[1] - pts_begin->at<cv::Vec3f>(y, x)[1]) / dt;
                }
            }
        }
                               ...
                               const bool use_polarity)
    {
        ...
        {
            const cv::Point2f &p = rectified_points[e->x + e->y * sensor_size.width];

            tl.x = int_floor(p.x);
            tl.y = int_floor(p.y);
            if (tl.x >= 0 && tl.x < sensor_size.width - 1 && tl.y >= 0 && tl.y < sensor_size.height - 1)
            {
